# local-file-system
基于服务器本地磁盘的单体文件存储服务（Spring Boot 2.6 + MyBatis-Plus，JDK11），面向 B2B 场景。
...
- **文件管理**：上传（直接或URL拉取）、按模块存储、下载、文件信息查询，记录文件名/类型/大小/相对路径/模块。
- **实体关联**：通过中间表将文件与任意业务实体关联，支持批量关联与按实体ID查询。
- **文件类型校验**：上传文件格式限制，当前支持 12 种格式，可自行添加（常见图片、PDF、Word/Excel/PPT），AOP 注解
  @CheckSupportUploadFileType 同时校验 MIME 和后缀。
- **数据迁移**：相对路径存储，迁移方式简单
